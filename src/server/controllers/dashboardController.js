...
const fetchDashboardData = async (userId) => {
  ...
  const loanStatsQuery = `
    SELECT 
      ...
      SUM(CASE WHEN status IN ('approved', 'account_manager', 'overdue') THEN 1 ELSE 0 END) as active_loans,
      SUM(CASE WHEN status IN ('approved', 'account_manager', 'overdue') THEN loan_amount ELSE 0 END) as total_loan_amount,
      SUM(CASE WHEN status IN ('approved', 'account_manager', 'overdue') THEN loan_amount ELSE 0 END) as outstanding_amount
  ...
  const activeLoansQuery = `
    ...
    FROM loan_applications
    WHERE user_id = ? AND status IN ('approved', 'account_manager', 'overdue')
