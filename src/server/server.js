...
const adminOtpSecurityRoutes = require('./routes/adminOtpSecurity');
...
const adminApiClientsRoutes = require('./routes/adminApiClients');
...
app.use(cors({
  ...
  allowedHeaders: ['Content-Type', 'Authorization', 'x-user-id', 'Accept', 'Origin', 'X-Requested-With', 'X-Dashboard-Request', 'x-dashboard-request'],
...
app.options('*', (req, res) => {
  ...
  res.header('Access-Control-Allow-Headers', 'Content-Type, Authorization, x-user-id, Accept, Origin, X-Requested-With, X-Dashboard-Request, x-dashboard-request');
...
app.use('/api/admin/otp-security', adminOtpSecurityRoutes);
...
app.use('/api/admin/api-clients', adminApiClientsRoutes);
...
app.use('/api/v1/bank-api', bankApiRoutes);
